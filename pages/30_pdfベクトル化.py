...
st.markdown("""
**処理の流れ:**
1. doc-manager-appの「PDFビューア（ocr）」で画像pdfをテキストpdfに変えてからこのベクトル化処理を行う  
...
# --- シャード選択 ---
previous_shard = st.session_state.get("previous_shard")  # 前回選択を保持
selected_shard = st.selectbox("対象シャード（年度）を選択", shards, index=0)

# --- シャード変更を検知して選択をリセット ---
if previous_shard != selected_shard:
    st.session_state["selected_pnos"] = []  # ← pno選択を空に戻す
    st.session_state["previous_shard"] = selected_shard  # 今回の選択を保存
...
if run:
    ...
    # ★ 今回処理したPDFを後でコスト集計に使うため記録
    processed_meta_keys: set[str] = set()   # "shard/pno/filename.pdf" などのフルキー
    processed_filenames: set[str] = set()   # ファイル名だけ
    ...
    if total_chunks > 0 and backend == "openai":
        ...
        # ★ “今回処理した全PDF” を対象に合算する（フルキー/ファイル名の両方でマッチ）
        ...
        summary = summarize_embedding_cost_from_meta(
            ...
            rate=DEFAULT_USDJPY,
            include_source_paths=include_list,  # ← 重要：全PDFの合計
            # created_after_iso=run_started_iso,  # metaにcreated_atがあれば実行開始時刻で絞り込むと更に厳密
        )
