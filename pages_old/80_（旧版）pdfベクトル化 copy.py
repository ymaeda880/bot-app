...
# ------------------------------------------------------------
# 📥 <PDF_ROOT>/<shard> を取り込み、
#    <VS_ROOT>/<backend>/<shard>/ に vectors.npy / meta.jsonl を追記。
#    meta には year / page / embed_model / shard_id / chunk_len_tokens 等を付与。
...
def migrate_processed_files_to_canonical(pf_json: Path, shard_id: str) -> None:
    ...
    - 'file.pdf' → 'shard/file.pdf'
    - dict 形式: file/path/name のいずれかに入っている場合に補完
    - 重複は除去
    """
...
run = st.button("選択した対象シャード内の PDF を取り込み", type="primary")
