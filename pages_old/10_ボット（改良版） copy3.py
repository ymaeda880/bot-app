...
# ストリーミングで「最初のトークンが到達した瞬間」を捕まえるラッパ
def stream_with_timing(gen, timings: Timings, first_key="gpt_first_token", done_key="gpt_done"):
    first_seen = False
    for chunk in gen:
        if not first_seen:
...
""".strip())
        with col3:
            st.write("**メモ**")
            st.caption("- *gpt_wait*: API 送信→最初のトークン到着まで\n- *gpt_stream*: 最初のトークン→完了まで\n- 一括表示では *gpt_wait* はほぼ *gpt_total* と同じ、*gpt_stream* は 0 に近くなります。")
